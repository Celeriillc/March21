...
public class ELibraryUploadBookActivity extends AppCompatActivity {
    ...
    EditText title, author, tags, description; //, approximateDuration
...
//                    String approximateDurationString = approximateDuration.getText().toString().trim();
                    String approximateDurationString = "0";
                    ArrayList<String> teacherList = new ArrayList<>();
                    teacherList.add(mFirebaseUser.getUid());
                    ageGradeString = ageGrade.getText().toString().trim();

                    String materialID = mFirebaseDatabase.getReference().child("E Library Private Materials").child("Teacher").child(mFirebaseUser.getUid()).push().getKey();

                    ELibraryMaterialsModel eLibraryMaterialsModel = new ELibraryMaterialsModel(materialID, mFirebaseUser.getUid(), titleString, descriptionString, authorString,
                            ageGradeString, downloadURL, "", fileType, "0", approximateDurationString, date, tagString, tagList, teacherList, schoolList);
        ...
        if (requestCode == 4) {
            if(resultCode == RESULT_OK) {
                if (data != null) {
                    ...
                    fileName.setText(getFileName(uri));
                    title.setText(getFileName(uri));
                    fileImage.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.ic_baseline_audio_headphones_purple_24));
                    fileLayout.setVisibility(View.VISIBLE);
                    tapToUploadLayout.setVisibility(View.GONE);
                    tapToUploadTextView.setVisibility(View.GONE);
                }
